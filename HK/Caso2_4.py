...
def Espiras(puntos):
	...
	w9c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=6*I_circulares).Translate([0,0,36.5])
	sol.AddWire(w9c)
	...
	w18c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=6*I_circulares).Translate([0,0,-35.5])
	sol.AddWire(w18c)
