...
void make_map3(TString inputname="1m/all1m.csv", TString outname="map.pdf", double kThd = 0.90,
	       Double_t hv = 1600.){//2000.) {
...
#else
  TH1D *heff = new TH1D("hEffLoss","Relative Detection Efficiency", 2060,0.6,1.03);
  heff->SetXTitle("Relative Detection Efficiency");
  TH1D *heffud[2];
  TH1D *hefftb[2];
  heffud[0] = new TH1D("hEffLossUp","Relative Detection Efficiency", 2060,0.6,1.03);
  heffud[1] = new TH1D("hEffLossDown","Relative Detection Efficiency", 2060,0.6,1.03);
  hefftb[0] = new TH1D("hEffLossTop","Relative Detection Efficiency", 2060,0.6,1.03);
  hefftb[1] = new TH1D("hEffLossBottom","Relative Detection Efficiency", 2060,0.6,1.03);
#endif
...
///   fin.clear();
  ...
  c->Print(Form("%s[", outname.Data()));
   ...
   DrawGeometry(10, eff, 0.87, 1.01, "", xyz);
   c->Print(Form("%s", outname.Data()));

   c->cd(0)->Clear();
   //DrawGeometry(1, eff, 0.97, 1.01, "", xyz);
   DrawGeometry(1, eff, 0.87, 1.01, "", xyz);
   c->Print(Form("%s", outname.Data()));
   c->SetLogy();
   heff->Draw();
   heffud[0]->Draw("same");
  ...
  double thd = kThd;
  double below_thd = 0.0;
  double entries   = 0.0;
  for (int j=0;j< heff->GetXaxis()->GetNbins();j++) {
    double eff = heff->GetXaxis()->GetBinCenter(j+1);
    entries += heff->GetBinContent(j+1);
    if (eff < thd) below_thd += heff->GetBinContent(j+1);
  }
  cout << Form("Threshold:%5.3f => All Entries:%8d, below thd:%8d, ratio below thd:% 7.4f",
	       thd,(int)entries,(int)below_thd, below_thd/entries) << endl;
//////////////////////////////////////////////////
