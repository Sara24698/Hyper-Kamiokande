...
# rectangular loops I=1 A
	...
	w1c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-35).Translate([0,0,pos_espira_circular[0]])
	w2c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-35).Translate([0,0,-34.5])
	w3c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-35).Translate([0,0,-33.5])
	w4c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-35).Translate([0,0,-32.5])
	w5c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-35).Translate([0,0,-31.5])
	w6c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-35).Translate([0,0,-30.5])
	w7c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-35).Translate([0,0,-29.5])
	w8c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-35).Translate([0,0,-28.5])
	sol.AddWire(w1c)
	sol.AddWire(w2c)
	sol.AddWire(w3c)
	sol.AddWire(w4c)
	...
	for j in range(4, len(pos_espira_circular)):
...
#Parte de abajo
	...
	Longitud3.append(2*np.pi*np.sqrt((9.05**2+19.05**2)/2))
	...
	sol.AddWire(w2e)
	Longitud3.append(2*np.pi*np.sqrt((14.765**2+24.77**2)/2))
	
	w3e = wire.Wire(path=wire.Wire.EllipticalPath(rx=23.5, ry=30.4, pts=20), discretization_length=0.1, current=-130).Rotate(axis=(0,0,1), deg=90).Translate([0,0,-35.5]).Translate([0, -9.6,0])#I = 40
	sol.AddWire(w3e)
	Longitud3.append(2*np.pi*np.sqrt((23.5*2+30.4**2)/2))
	Longitud3.append(2*np.pi*np.sqrt((23.5*2+30.4**2)/2))

	
	w4e = wire.Wire(path=wire.Wire.EllipticalPath(rx=9.05, ry=19.05, pts=20), discretization_length=0.1, current=80).Rotate(axis=(0,0,1), deg=90).Translate([0,0,-35.5]).Translate([0,24.7,0])
	sol.AddWire(w4e)
	Longitud3.append(2*np.pi*np.sqrt((9.05**2+19.05**2)/2))
	...
	sol.AddWire(w5e)
	Longitud3.append(2*np.pi*np.sqrt((23.7**2+31.4**2)/2))
	Longitud3.append(2*np.pi*np.sqrt((23.7**2+31.4**2)/2))
...
print(Longitudes1, Longitudes2, Longitudes3)
