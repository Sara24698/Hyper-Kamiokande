# -*- coding: utf-8 -*-
"""
Created on Mon Mar 18 16:12:07 2024
...
#I_rectangular = 85
#I_circulares = -80
I_rectangulares=np.array([ -88.80043015,  -86.69853635,  -80.98850459,  -78.00079271,  -75.31136559, -73.54398473,  -72.31080865,  -71.5302371,   -70.01047137,  -70.03898655, -68.93096188,  -69.2616355,   -68.1624593,   -68.08474841,  -69.03814609, -64.90237085,  -73.01204377,  -64.95610797,  -68.48812072,  -67.71353784, -67.77623694,  -68.51152183,  -68.03025032,  -69.03816138,  -68.52356728, -70.16051085,  -70.33699906,  -71.26267128,  -72.80446878,  -75.72836772, -79.98543286,  -88.07865206,  -89.64634082])
I_circulares=np.array([-142.65180479,  102.71607186, 80.46722376,   83.85007211,   81.66739748,   80.85583209,   79.9571072, 79.28260443,   78.7334496,    78.29931109,   77.96391448,   77.70994467, 77.53007555,   77.41795415,   77.36647253,   77.37823345,   77.44893134, 77.58359075,   77.78644274,   78.0614929,    78.42214275,   78.87779, 79.43329945,   80.15575745,   80.79414895,   82.4089002,    79.73256229, 95.94343019,  -46.72938197,  675.20067806,  657.06137959])
...
radio_tapas = 26
radio_tapas2 = 20
pos_espira_circular = np.arange(-33.6,34,2.4)
...
def Espiras(puntos):
	...
	w2r = wire.Wire(path=wire.Wire.RectangularPath(dx=np.sqrt(radio_cilindro**2 - pos_espira_rectangular[0]**2)*2, dy=Altura), discretization_length=0.1, current=-I_rectangulares[-1]).Rotate(axis=(1, 0, 0), deg=-90).Translate([0,pos_espira_rectangular[-1], 0]).Translate([0,0, 0.5])
	sol.AddWire(w2r)
	Longitud.append(np.sqrt(radio_cilindro**2 - pos_espira_rectangular[0]**2)*4+Altura*2)
	...
	w9c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-I_circulares[-2]).Translate([0,0,36.5])
	sol.AddWire(w9c)
	Longitud.append(2*2*np.pi*radio_cilindro)
	
	
	#w1c = wire.Wire(path=wire.Wire.CircularPath(radius=26, pts=20), discretization_length=0.1, current=I_circulares).Translate([0,0, -35.5])
	#sol.AddWire(w1c)
	#Longitud.append(2*np.pi*27)
	#w2c = wire.Wire(path=wire.Wire.CircularPath(radius=24, pts=20), discretization_length=0.1, current=I_circulares).Translate([0,0, 36.5])
	#sol.AddWire(w2c)
	#Longitud.append(2*np.pi*27)
	...
	w18c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-I_circulares[-1]).Translate([0,0,-35.5])
	sol.AddWire(w18c)
	...
	if len(puntos) == len(PMTs_top):#3214:#6437:
...
#df.to_csv('./Caso2_4.csv', index = False)
...
textstr = '\n'.join((r'$\mu=%.2f$' % (Media_total, ), r'$\sigma=%.2f$' % (Desviacion_est, ), r'Prop.excess=%.2f' % (Porcentaje, )))
