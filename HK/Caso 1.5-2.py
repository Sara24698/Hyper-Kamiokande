...
radio_tapas = 27
radio_tapas2 = 20
pos_espira_circular = np.arange(-35.5,37,1.5)
...
# rectangular loops I=1 A
	...
	w9c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=7*I_circulares).Translate([0,0,pos_espira_circular[len(pos_espira_circular)-1]])
	sol.AddWire(w9c)



	w15c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=8*I_circulares).Translate([0,0,pos_espira_circular[0]])
	sol.AddWire(w15c)
...
#Parte de abajo
	...
	w3e = wire.Wire(path=wire.Wire.EllipticalPath(rx=23.5, ry=30.4, pts=20), discretization_length=0.1, current=50).Rotate(axis=(0,0,1), deg=90).Translate([0,0,-35.5]).Translate([0, 9.6,0])#I = 40
	sol.AddWire(w3e)
	...
	w5e = wire.Wire(path=wire.Wire.EllipticalPath(rx=23.7, ry=31.4, pts=20), discretization_length=0.1, current=100).Rotate(axis=(0,0,1), deg=90).Translate([0,0,36.5]).Translate([0, -8.5,0])
	#sol.AddWire(w5e)
	Longitud.append(2*np.pi*np.sqrt((20.48**2+30.49**2)/2))
