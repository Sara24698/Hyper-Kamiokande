...
# rectangular loops I=1 A
	...
	w18c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-85).Translate([0,0,pos_espira_circular[0]])#I_circulares
	sol.AddWire(w15c)
	...
	w19c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-72).Translate([0,0,pos_espira_circular[len(pos_espira_circular)-4]])#I=70
	sol.AddWire(w19c)
	Longitud.append(2*np.pi*radio_cilindro)
	Longitud.append(2*np.pi*radio_cilindro)


	w20c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-68).Translate([0,0,pos_espira_circular[1]])
	sol.AddWire(w20c)
	...
	w22c = wire.Wire(path=wire.Wire.CircularPath(radius=radio_cilindro, pts=20), discretization_length=0.1, current=-75).Translate([0,0,pos_espira_circular[len(pos_espira_circular)-5]])
	sol.AddWire(w22c)
...
df.to_csv('/home/sara/Sara/Caso 1m-2m.csv', index = False)
