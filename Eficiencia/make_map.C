...
void make_map(TString inputname="1m/all1m.csv", TString outname="map.pdf", Double_t hv = 1600.){//2000.) {
...
#ifdef takeminimum
  TH1D *heff = new TH1D("hEffLoss","Relative Detection Efficiency", 200,0.95,1.01);
  heff->SetXTitle("Relative Detection Efficiency");
  TH1D *heffud[2];
  TH1D *hefftb[2];
  heffud[0] = new TH1D("hEffLossUp","Relative Detection Efficiency", 200,0.95,1.01);
  heffud[1] = new TH1D("hEffLossDown","Relative Detection Efficiency", 200,0.95,1.01);
  hefftb[0] = new TH1D("hEffLossTop","Relative Detection Efficiency", 200,0.95,1.01);
  hefftb[1] = new TH1D("hEffLossBottom","Relative Detection Efficiency", 200,0.95,1.01);
#else
  TH1D *heff = new TH1D("hEffLoss","Relative Detection Efficiency", 200,0.95+0.015,1.01+0.015);
  heff->SetXTitle("Relative Detection Efficiency");
  TH1D *heffud[2];
  TH1D *hefftb[2];
  heffud[0] = new TH1D("hEffLossUp","Relative Detection Efficiency", 200,0.95+0.015,1.01+0.015);
  heffud[1] = new TH1D("hEffLossDown","Relative Detection Efficiency", 200,0.95+0.015,1.01+0.015);
  hefftb[0] = new TH1D("hEffLossTop","Relative Detection Efficiency", 200,0.95+0.015,1.01+0.015);
  hefftb[1] = new TH1D("hEffLossBottom","Relative Detection Efficiency", 200,0.95+0.015,1.01+0.015);
#endif
...
///   fin.clear();
  ...
  c->Print(Form("%s[", outname.Data()));
   ...
   Long64_t totalpmt = t->Draw("TMath::Sqrt(Bx*Bx+By*By+Bz*Bz)>>hBtotal(100,0,200)", "", "");
   t->Draw("TMath::Sqrt(Bx*Bx+By*By+Bz*Bz)>>hBtotalTop(100,0,200)","faceid==1","same");
   t->Draw("TMath::Sqrt(Bx*Bx+By*By+Bz*Bz)>>hBtotalBarrel(100,0,200)","faceid==2","same");
   t->Draw("TMath::Sqrt(Bx*Bx+By*By+Bz*Bz)>>hBtotalBottom(100,0,200)","faceid==3","same");
